...
import { Test, Question } from '@/services/geminiService';
...
const TestPreview = ({ test }: TestPreviewProps) => {
    ...
    <div key={index} className="group">
      <div className="p-4 space-y-2">
        <div className="flex gap-2">
          <span className="flex-shrink-0 w-8 h-8 bg-lavender-100 rounded-full flex items-center justify-center text-sm font-medium text-lavender-600">
            {index + 1}
          </span>
              ...
              <div className="pl-5 space-y-2 mt-2">
                {question.options.map((option, optIndex) => (
                  <div key={optIndex} className="flex items-start gap-2">
                    <span className="text-sm font-medium text-lavender-600">{String.fromCharCode(65 + optIndex)}.</span>
      ...
      <Separator className="last:hidden bg-lavender-100" />
    </div>
  );
  ...
  return (
    <div className="w-full animate-fade-in print:animate-none">
      <Card className="bg-white/95 backdrop-blur-sm border border-lavender-200 shadow-lg print:shadow-none rounded-3xl">
        <CardHeader className="pb-3">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
              <div className="text-xs font-medium text-lavender-500 mb-1 print:hidden">
                Test Preview
              </div>
              <CardTitle className="text-2xl text-lavender-800">{test.title}</CardTitle>
            </div>
            <Button 
              onClick={handlePrint}
              variant="outline" 
              className="self-start print:hidden button-animation border-lavender-200 text-lavender-700 hover:bg-lavender-50"
            >
              Print Test
            </Button>
          </div>
            ...
            <TabsList className="w-full h-auto flex flex-wrap gap-2 bg-transparent pb-1 justify-start">
              {groups.map(group => (
                <TabsTrigger 
                  key={group.id} 
                  value={group.id}
                  className="px-3 py-1.5 h-auto data-[state=active]:bg-lavender-100 data-[state=active]:text-lavender-800 data-[state=active]:shadow-sm"
                >
                  {group.name}
                </TabsTrigger>
              ))}
